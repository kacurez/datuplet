...
cpu: Apple M3
BenchmarkInstantiate-8   	   48901	     62366 ns/op	  251862 B/op	    1570 allocs/op
BenchmarkEval300KB-8     	      56	  62144247 ns/op	30256400 B/op	    3872 allocs/op
    bench_test.go:110: generated bundle: 307365 bytes   (x4)
BenchmarkJSON10MiB-8     	       1	4660615292 ns/op	871959760 B/op	 1068358 allocs/op
    bench_test.go:145: generated query response: 10485810 bytes
PASS
ok  	github.com/datuplet/datuplet/pkg/appengine	17.869s
```
...
pkg: github.com/datuplet/datuplet/pkg/appengine
cpu: Apple M3
BenchmarkJSON10MiB-8   	       4	4453948167 ns/op	764732884 B/op	  268327 allocs/op
    bench_test.go:145: generated query response: 10485810 bytes   (x3)
PASS
ok  	github.com/datuplet/datuplet/pkg/appengine	36.403s
```

The 4-rep average (4.45 s/op) is consistent with the single-shot sample
(4.66 s/op) from the first pass — no wild variance.

### Headline numbers

| Benchmark | Time/op | Bytes/op (Go host allocs) | Allocs/op |
|---|---|---|---|
| `BenchmarkInstantiate` | **0.062 ms** | 252 KB | 1,570 |
| `BenchmarkEval300KB` | **62.1 ms** | 30.3 MB | 3,872 |
| `BenchmarkJSON10MiB` | **4.45 s** (avg of 4) | 765 MB | 268,327 |
...
**Fixed floor (`BenchmarkInstantiate`, 62 µs).** Negligible. It confirms the
E1-era design decision (compile once in `NewEngine`, fresh instance per
`Render`) does not reintroduce a meaningful per-render tax — instantiation +
`_initialize` costs three orders of magnitude less than a typical render.
No case for instance pooling on these numbers.

**Typical-bundle eval (`BenchmarkEval300KB`, 62.1 ms, no query).** This
...
back-to-back with no query wait, 8 concurrent slots at 62.1 ms/render would
imply a theoretical ceiling of:

```
8 slots / 0.0621 s ≈ 129 renders/s/pod
```
...
```
8 slots / (0.062 s eval + ~1 s query wait) ≈ 7.5 renders/s/pod
```
...
**Render-slot math, heavy-JSON path (`BenchmarkJSON10MiB`, 4.45 s).** This
...
pattern) costs **4.45 s of single-threaded guest CPU time alone** — before
any actual network/query-execution latency is added on top. Against the
default 10 s wall clock, that consumes ~44.5% of the render's entire time
...
```
8 slots / (4.45 s eval + ~1 s query wait) ≈ 1.5 renders/s/pod
```

— roughly **7–20x below** the spec's assumed 10–30 renders/s for any app
...
- Go-host allocations scale with payload, as expected: 252 KB (no eval) →
  30.3 MB (300 KB bundle text + one JS heap) → 765 MB (10 MiB response
  ...
  765 MB of *host*-side churn per render is a real GC-pressure number the
...
1. **Fixed per-render overhead is a non-issue.** 62 µs to instantiate a
...
2. **Typical-app eval cost is well within budget.** 62 ms for a 300 KB
...
3. **The 10 MiB-reshape path is real but bounded and known.** 4.45 s of
...
2. **`BenchmarkJSON10MiB`'s `MaxOutputBytes`** is set to 64 MiB, far above
   ...
   the 4.45 s figure as an upper bound on guest CPU cost for that class of
