...
- [ ] **Step 1: Pin the kubectl helper image.** Bitnami retired versioned `bitnami/kubectl` tags (only `:latest` + `bitnamilegacy/kubectl:X.Y` remain). **RESOLVED DURING EXECUTION → use `alpine/k8s:1.31.12`, NOT `bitnamilegacy/kubectl`.** The `bitnamilegacy/kubectl` images use dash as `/bin/sh`, which rejects `set -o pipefail`; the keygen/authz/openfga-dsn bootstrap Jobs all start `set -euo pipefail`, so the legacy image fails them (`sh: set: Illegal option -o pipefail` → `BackoffLimitExceeded` → helm pre-install hook fails). A live `make e2e-k8s-deploy` reproduced this. `alpine/k8s:1.31.12` is a multi-arch kubectl toolbox whose busybox `ash` supports pipefail (verified in-cluster). Apply it to **all four sites**: `charts/datuplet-app/values.yaml` + `charts/datuplet-infra/values.yaml` (`bootstrap.externalImages.kubectl`, update the explanatory comments at both) and `charts/datuplet-lakekeeper/values.yaml` (the two `extraInitContainers[].image`). Regenerate the lakekeeper `_render/*.golden.yaml` fixtures (`make chart-render-check` to verify). Multi-arch sanity check for any kubectl toolbox image: `docker manifest inspect docker.io/<image> | grep -c 'arm64\|amd64'` ≥2.
...
```
  ...
  `cleanup()` must also kill the supervisors' children — **reap the child BEFORE the parent** (killing the supervisor first orphans its `kubectl port-forward` child to PID 1 on Linux), and make BOTH kills best-effort so `set -e` doesn't abort the trap mid-loop when one finds nothing. Use an if-block:
  ```bash
  if [ -n "${pid:-}" ]; then
