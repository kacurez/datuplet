...
- [ ] **Step 1: Failing tests** (`resource_clamp_test.go`), minimum:
  ...
  Plus an **admission** test: a Pipeline component with over-max spec resources → after `handlePending` admission the frozen `pr.Status.ResolvedSpec` component's `Resources` are clamped and the run `status.message` records the note; and a `buildComponentJob` verbatim-consumption test: given an already-clamped `resolvedSpec`, the produced Job container's `Resources` equal the snapshot value byte-for-byte (build adds no clamp of its own).
- [ ] **Step 2: Implement.** Add `resource_clamp.go`. **T1 recon correction (drift — resources are NOT on the run snapshot):** `datupletv1.ResolvedComponentStatus` (`pr.Status.Components[]`) carries only `{Name, Component, Version, Image, ImageID}` — it has **no `Resources` field**. The registry `ComponentResources{Default,Max}` are reachable ONLY at admission, off `rc` returned by `r.Registry.Resolve(...)`. So compute effective resources in `pkg/k8s/controllers/pipelinerun_controller.go`'s admission loop inside `handlePending` (the `for i := range resolvedSpec.Stages { for j := range stage.Components { rc, findings := r.Registry.Resolve(comp.Component, comp.Version) … } }` block ~:424–449, where `comp := &stage.Components[j]` is a pointer INTO `resolvedSpec` and `rc.Resources.Default/.Max` are in hand): call `applyDefaultsThenClamp(comp.Resources, rc.Resources.Default, rc.Resources.Max)`, set `comp.Resources = &result.Resources` (mutating the frozen `resolvedSpec` component before `pr.Status.ResolvedSpec = resolvedSpec` at ~:450), and collect each non-empty `result.Note` for the run `status.message`. In `buildComponentJob` (`pkg/k8s/controllers/pipelinerun_jobs.go` :270–271) do **NOT** add clamp logic — the existing `if comp.Resources != nil { container.Resources = *comp.Resources }` already consumes the frozen `resolvedSpec` value verbatim (its `comp` is `pr.Status.ResolvedSpec.Stages[].Components[]`, passed by the `startStage`/`checkStageComponents` callers at ~:737-741 / :811); leave it as the single point of resource application. The Files "delete any resource fallback logic" note means there is no other resource branch to remove — pre-Phase-2's "nil = unlimited" is already gone.
